...
def huawei_crawl(page=1):
    ...
    while True:
          ...
          hasNextPage = game_json.get('hasNextPage')
          print('hasNextPage %s  totalPages %s'%(hasNextPage,totalPages))
          if hasNextPage == 0:
            break
          ...
          for game in game_data:
              ...
              row_data = ['华为新游预约', day, test_time, name, game_type, developer, None,
                          ORDERUSER, None, detailId, icon, screenshots, description, download]
...
all_df.to_excel('开测华为_{date}.xlsx'.format(date=today), index=False)
