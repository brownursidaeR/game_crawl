...
# print(response.text)
rencoding = chardet.detect(response.text)
if rencoding[''] = 'utf-8'

# game_json = response.text
# print(game_json)
# with open('huawei_rank.json','w',encoding='utf-8') as f:
#     f.write()


data = json.loads(game_json)['layoutData']
game_data = data[0]['dataList']
game_list = []
for game in game_data:
    ...
    row_data = ['华为新游预约',day,test_time,name,game_type,developer,None,ORDERUSER,None,detailId,icon,description,download]
    if row_data:  # Only append if there's data
        game_list.append(row_data)

headers = ['数据来源','测试日期','时间','游戏名','游戏类型','发行厂商','研发厂商','平台评分','测试模式','详情页','icon','游戏介绍','下载链接']
